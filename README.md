...
This repository is based on work from [yxyang](https://github.com/yxyang). It adapted the whole_body_controller_example to use its data for Inverse Reinforcement Learning on the Unitree A1. The new model walks with a maximum speed in the desired direction and interprets the direction in the absolute coodinate system  instead of from the point of view of the robot. The previous control method led to an accumulating errors in the direction and to unusable data for our purposes. For further details have a look at my thesis. The generated dataset are at [here](https://drive.google.com/drive/folders/1w5SeejITkFCH0KgEUUuaGRv6MRdIiPFo?usp=sharing). The dataset describtion is as follows:
